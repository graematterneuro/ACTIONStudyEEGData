...
for row = 1:NoRows
    ...
    while repeatWReject == 1
        ...
        if passCounter == 1
            ...
            lineNoiseChk  = input(append("Is there line noise which requires cleaning?", ...
                newline, "[Y = 1/N = anything else]", newline));

            if lineNoiseChk == 1

                % Plot channel spectra close up to determine line noise frequencies
                multiFreq = input(append("Is there more than one frequency to clean?", ...
                    newline, "[Y = 1/N = anything else]", newline));
            ...
            % Code on lines 331 to 342 (inclusive) merged from pop_subcomp.m
            ...
            % Display channel graphs so that noisy channels can be
            % identified
            figure; pop_spectopo(EEG, 1, ...
                [0  119998], ...            % Epoch time range to analyze [min_ms max_ms]
                'EEG' , ...
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            % channel structure - Needed for eeg_icainterp() on line 530
            ...
            % Plot single trials before and after IC rejection.
            % Code on lines 555 to 566 (inclusive) merged from pop_subcomp.m
            % (https://github.com/sccn/eeglab/blob/develop/functions/popfunc/pop_subcomp.m)
            components = [];
            components = find(EEG.reject.gcompreject == 1);
            components = components(:)';
